 ...
 */
let Container = function Container(require, Definition) {
    this.parameters = {};
    this.services = {};
...
Container.prototype.constructService = function constructService(id, isOptional, serviceTree, ns) {
    let i, definition, arguments, argument, svc, calls, properties, classFile,
        namespace;
    ...
    if (!definition.class) {
        switch(true) {
            case this.isArgumentALiteral(definition.file):
            case definition.file.charAt(0) === '@':
    ...
    if (definition.isObject) {
        ...
        constructor = definition.class[definition.constructorMethod];
        svc = Reflect.construct(constructor, arguments);
    } else {
 ...
 * @returns {undefined}
 */
Container.prototype.setDefinition = function (id, serviceDefinition, scope) {
 ...
 * @returns {undefined}
 */
Container.prototype.set = function (id, service, scope) {
    let definition = new this.Definition();
...
Container.prototype.getParameter = function (name, ns) {
    ns = ns || '';
    let parameter = this.parameters[ns + name];
